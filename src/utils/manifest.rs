...
use crate::utils::{manifest, util};
use crate::utils::util::exit_with_error;
...
pub fn get_file_manifest(path: &str, r: &Replace, gzip: bool) -> FileManifest {
    ...
    if gzip && FILEEXTENSTIONTOGZIP.contains(&file_extension.as_str()) {
        ...
        if !util::copy_file(&path, &new_path) {
            exit_and_error!(
                    "Failed to copy file {} to {}", path, new_path
            );
        }
        file_manifest.size = util::file_size(&new_path);
    }
...
pub fn get_folder_manifest(path: &str, r: &Replace, gzip: bool) -> FolderManifest {
    let new_path = path.replace(&r.replace, &r.to).to_string();
    ...
    for entry in WalkDir::new(path).max_depth(1).contents_first(true) {
        ...
        if entry_path.is_dir() {
            ...
            let f_manifest = get_folder_manifest(entry_path_str, r, gzip);
            ...
            let file_manifest = get_file_manifest(entry_path_str, r, gzip);
            // update folder size and children
...
pub fn get_manifest(start_path: &str, output: &str, gzip: bool) -> FolderManifest {
    get_folder_manifest(
        start_path,
        &Replace {
            to: output.to_string(),
...
pub fn extract_manifest_from_zip(zip: &File) -> FolderManifest {
    ...
    match zip_file {
        Ok(c) => {
            serde_json::from_slice(c.as_slice()).unwrap_or_else(|e| {
                exit_and_error!(
